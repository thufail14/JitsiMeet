    ...
    return (dispatch, getState) => {
        ...
        dispatch({
            ...
            participant: {
                conference,
                id,
                appJid: (participant && participant.appJid) || undefined
            }
        });
    };
}
...
export function participantAppJidUpdated(participant = {}) {
    return (dispatch, getState) => {
        if(participant.id){
            const participantDetail = getParticipantById(getState, participant.id);
            if(participant.appJid && participantDetail.appJid != participant.appJid){
                const participantToUpdate = {
                    ...participant
                };
                
                dispatch({
                    type: PARTICIPANT_APP_JID_UPDATED,
                    participant: participantToUpdate
                });
            }
        }
