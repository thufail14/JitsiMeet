...
export function sendLocalParticipant(
    ...
    appJid && conference.sendCommand(APP_JID_COMMAND, {
        value: appJid
    });
...
/**
 * Generate the call type name based on call mode
 * @param {*} _callMode 
 */
export function getCallTypeName(_callMode){
    return (_callMode === 'onetomany') ? 'conference' : (_callMode === 'manytomany' ? 'meeting' : 'call')
}

/*
 * Generate room name with 3-4-3 pattern;
 * 
 * @param {*} length 
 */
export function generateRoomName(length = 10){
    let result = '';
    ...
    result = result || 'cd3-dFz6-g5Y';
    return result;
}

export function isValidRoomNamePattern(roomName){
    if(roomName){
        return roomName.indexOf("-") == 3 && roomName.lastIndexOf("-") == 8 && roomName.length == 12;
    }
    return true;
}
