...
import { CALL_MODE_ONETOONE, CALL_MODE_CONFERENCE } from '../base/conference';
import { CALLING, RINGING, CALL_TYPE_OUTGOING } from "../presence-status";
...
/**
 * Format the callinfo users name with comma separator
 * 
 * @param {*} users 
 */
export function getCallInfoUsersName(users: array){
...
/**
 * Get the avatar url of single user call
 * @param {*} users 
 */
export function getCallInfoUserAvatar(users: array){
    if(users && Array.isArray(users) && users.length > 0){
        return users.length === 1 ? users[0].avatar : null;
    }
    return null;
}

/**
 * Check whether given callmode is required ringing sound or not
 * @param {*} callMode
 */
export function isCallModeNeedRing(callMode: string){
    return callMode ? [CALL_MODE_ONETOONE, CALL_MODE_CONFERENCE].indexOf(callMode) > -1 : false;
}

/**
 * Check whether call status is still ringing
 * @param {*} callStatus 
 */
export function isStillRinging(callStatus: string){
    return callStatus ? [CALLING, RINGING].indexOf(callStatus) > -1 : false;
}

export function isOutGoingCall(callType: string){
    return callType === CALL_TYPE_OUTGOING;
}
