...
MiddlewareRegistry.register(store => next => action => {
    ...
    case ENTER_PICTURE_IN_PICTURE:
        const dimension = (action && action.dimension) || {};
        sendEvent(store, type, /* data */ dimension);
        break;
    ...
    case CONFERENCE_WILL_LEAVE:
        const userInfo = store.getState()['features/base/settings'];
        if(userInfo && userInfo.appJid){
            sendEvent(store, type, {
                appJid:userInfo.appJid
            });
        }
        break;
    case PARTICIPANT_APP_JID_UPDATED:
        if(action.participant && action.participant.id && action.participant.appJid){
            const participantCount = getParticipantCount(store);
            sendEvent(store, 'JOINED_PARTICIPANT_APP_JID', {
                appJid:action.participant.appJid,
