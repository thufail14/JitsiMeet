...
import { setRoom, isValidRoomNamePattern } from '../base/conference';
import {
    configWillLoad,
    createFakeConfig,
    loadConfigError,
...
import { showNotification, clearNotifications } from '../notifications';
...
if(navigator.product != 'ReactNative'){
    import('toastr').then((toast) => toastr=toast);
}
...
export function appNavigate(uri: ?string) {
    return async (dispatch: Dispatch<any>, getState: Function) => {
        ...
        if(navigator.product != 'ReactNative' && !isValidRoomNamePattern(location.room)){
            if(toastr.error){
                toastr.error('Invalid meeting URL.');
            }
