...
internal final class JMCallKitEmitter: NSObject, CXProviderDelegate {
    ...
    private var pendingHoldActions = Set<UUID>()
    ...
    // MARK: - Add hold action
    
    func addHoldAction(_ actionUUID: UUID) {
        pendingHoldActions.insert(actionUUID)
    }
    ...
    func provider(_ provider: CXProvider, perform action: CXSetHeldCallAction) {
        let uuid = pendingHoldActions.remove(action.uuid)
        
        // Avoid hold actions ping-pong: if the hold action was caused by
        // the JS side (we requested a transaction) don't call the delegate
        // method. If it was called by the provder itself (when the user presses
        // the hold button in the CallKit view) then call the delegate method.
        //
        // NOTE: don't try to be clever and remove this. Been there, done that.
        // Won't work.
        if (uuid == nil) {
            listeners.forEach {
                let listener = $0 as! JMCallKitListener
                listener.performSetHoldedCall?(UUID: action.callUUID, isHolded: action.isOnHold)
            }
        }
